...
const TvNav = ()=>{
   ...
   <Link to={"/browse"}><li className="cursor-pointer text-md p-1 m-1 hover:text-slate-500">Home</li></Link>
    <Link to={"/Tv"}><li className=" cursor-pointer text-md p-1 m-1 hover:text-slate-500">TV</li></Link>
    <Link to={"/browse"}><li className=" cursor-pointer text-md p-1 m-1 hover:text-slate-500">Movie</li></Link>
    <Link to={"/watchlist"}  ><li className="cursor-pointer text-md p-1 m-1 hover:text-slate-500" >My list</li></Link>
   </ul></div>
   <div className="border border-red-600 w-4/12 flex justify-around items-center">
   {
    Location.pathname!=="/watchlist" ? <div>{IsTVCard?<button className="text-white bg-red-600 px-4 py-1 rounded-md cursor-pointer" onClick={()=>dispatch( AddShowTvCard(false))} >Search</button>:
    <button className="text-white bg-red-600 px-4 py-1 rounded-md cursor-pointer" onClick={()=>dispatch( AddShowTvCard(true))} >Tv Series</button>} </div>:" "
   }
    ...
    <button  className="text-white bg-slate-400 px-4 py-1 rounded-md cursor-pointer" onClick={()=>SignOutHandler()}> Sign out</button>
    </div>
   </div>
    </nav> )
}
